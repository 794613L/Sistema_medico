...
export class AtencionMedicalComponent {
  ...
  ngOnInit(): void {
    ...
    this.appointmentService.showAppointmentAttention(this.appointment_id).subscribe((resp:any) => {
      ...
      this.medical = this.appointment_attention_selected.recipe_medica;
  ...
  save(){
    ...
    if(!this.description || this.medical.length == 0){
      this.text_validation = "ES NECESARIO LLENAR EL DIAGNOSTICO Y UNA RECETA MEDICA";
      return;
    }
    ...
    this.appointmentService.registerAttention(data).subscribe((resp:any) =>{
      console.log(resp);
      this.text_success = "LA INFORMACIÓN DE LA ATENCIÓN MEDICA SE GUARDO CORRECTAENTE";
    })
